...
- **장학금 GPA 지킴이**: 목표 섹션에 장학금 기준 카드 추가 — 현재 누적 GPA와 비교해 유지/위험/미달 상태를 실시간 표시. 여러 장학금 기준 등록 가능.
- **전공/교양 분리 GPA**: GPA 요약 카드 하단에 전공·교양 GPA 칩 표시. 과목 type 데이터 활용, 해당 유형 과목 없을 때는 자동 숨김.
- **재수강 효과 시뮬레이터**: 목표 섹션에 시뮬레이터 카드 추가 — 과목 선택 + 목표 성적 입력만으로 누적 GPA 변화 즉시 계산. A+ 재수강 시 효과 큰 과목 3개 자동 추천.

### 빠른 개선 4종

- **과목 메모**: 편집 모달에 메모 필드 추가 (최대 100자), 과목 행에 이탤릭 표시.
- **학기 복사**: 탭의 복사 버튼으로 현재 학기 과목을 포함한 새 학기 즉시 생성.
- **인사이트 카드**: 성적 시각화 섹션에 GPA 추이·전공/교양 비교·통계 인사이트 자동 생성.
- **스와이프 삭제**: 과목 행 좌로 스와이프 → 빨간 삭제 영역 노출 (iOS 스타일, 기존 버튼 병행).

### 재수강 마킹

- 편집 모달에 "재수강 과목" 토글 추가 — 과목 행에 주황 "재수강" 뱃지 표시.
